...
for tree in place_outlier/augmented_tree*nwk; do
    python ~/my_gits/LAML/run_laml.py -c ../../bar21/Bar21_character_matrix.txt -t $tree -o place_outlier/`basename $tree .nwk`_optimized -v -p ../../bar21/Bar21_priors.pickle --delimiter tab -m -1 --nInitials 1
done
