setwd("/Users/gc3045/git/laml-experiments/sim_tlscl/Results/imputation")

require(ggplot2)

d = read.table("problin_impute_missing.txt",header=T)
d$model = factor(d$model,levels=c("s0d100","s25d75","s50d50","s75d25","s100d0"),labels=c("h0d100","h25d75","h50d50","h75d25","h100d0"))
d$type = factor(d$type,levels=c("trueTopoTrueParams","trueTopo","estTopo"),labels = c("True tree","True topology","Estimated topology"))

okabe_ito <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442",
               "#0072B2", "#D55E00", "#CC79A7", "#000000")  # 8-colour set

ggplot(d, aes(x = model, y = accuracy, fill = type)) +
  geom_boxplot(outlier.shape = NA) +                         # cleaner boxes
  stat_summary(                                              # add group means
    fun   = mean,
    geom  = "point",
    shape = 23, size = 3, stroke = 0.3, fill = "white",
    position = position_dodge2(width = 0.75)
  ) +
  scale_fill_manual(values = okabe_ito[seq_len(n_distinct(d$type))],
                    name   = NULL) +                         # no legend title
  ylab("Accuracy") +
  theme_classic() +
  theme(
    legend.position  = "bottom",
    axis.title.x     = element_blank(),
    legend.title     = element_blank()
  )

ggsave("missing_impute.cbf.pdf",width=4.1,height = 4)

round(mean(d[d$model=="h0d100",]$accuracy),2); round(sd(d[d$model=="s0d100",]$accuracy),3)
round(mean(d[d$model=="h25d75",]$accuracy),2); round(sd(d[d$model=="s25d75",]$accuracy),3)
round(mean(d[d$model=="h50d50",]$accuracy),2); round(sd(d[d$model=="s50d50",]$accuracy),3)
round(mean(d[d$model=="h75d25",]$accuracy),2); round(sd(d[d$model=="s75d25",]$accuracy),3)
round(mean(d[d$model=="h100d0",]$accuracy),2); round(sd(d[d$model=="s100d0",]$accuracy),3)
